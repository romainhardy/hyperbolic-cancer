...
def poincare_distance_c(x: Tensor, y: Tensor, c: Tensor, keepdim: bool = True, **kwargs: Any) -> Tensor:
    # Using manual implementation instead of geoopt
    sqrt_c = sqrt(c)
    poincare_ball = geoopt.PoincareBall(c=c)
    mob = poincare_ball.mobius_add(-x, y, dim=-1).norm(dim=-1, p=2, keepdim=keepdim)
    arg = sqrt_c * mob
    dist_c = atanh(arg)
    res = dist_c * 2 / sqrt_c
...
def parallel_transport_mu0(x: Tensor, dst: Tensor, radius: Tensor) -> Tensor:
    poincare_ball = geoopt.PoincareBall(c=_c(radius))
    return poincare_ball.parallel_transport0(dst, x)


def inverse_parallel_transport_mu0(x: Tensor, src: Tensor, radius: Tensor) -> Tensor:
    poincare_ball = geoopt.PoincareBall(c=_c(radius))
    return poincare_ball.parallel_transport0back(src, x)
...
def exp_map_c(x: Tensor, at_point: Tensor, c: Tensor) -> Tensor:
    poincare_ball = geoopt.PoincareBall(c=c)
    return poincare_ball.expmap(at_point, x)
...
def exp_map_mu0_c(x: Tensor, c: Tensor) -> Tensor:
    poincare_ball = geoopt.PoincareBall(c=c)
    return poincare_ball.expmap0(x)
...
def inverse_sample_projection_mu0(x_proj: Tensor, at_point: Tensor, radius: Tensor) -> Tuple[Tensor, Tensor]:
    ...
    x = v_ * poincare_ball.lambda_x(at_point, dim=-1, keepdim=True)  # Corresponds to PT multiplied by 2.
    return v_, x
...
def exp_map_x_polar(x: Tensor, radius: Tensor, v: Tensor, c: Tensor) -> Tensor:
    ...
    poincare_ball = geoopt.PoincareBall(c=c)
    return poincare_ball.mobius_add(x, second_term)
