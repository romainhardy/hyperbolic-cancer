...
def run_experiment(X_train, y_train, cluster_assignments_train, X_valid, y_valid, num_clusters):
    # Sample random mixture of clusters
    mixture_vector = sample_random_mixture(num_clusters, 25000)
    X_samples = []
    y_samples = []
    ...
    for epoch in range(num_epochs):
        ...
        print(f"epoch {epoch + 1:03d} / {num_epochs:03d} || loss: {average_loss:.4f} || valid_loss: {avg_valid_loss:.4f} || valid_accuracy: {valid_accuracy:.2f}% || valid_f1: {valid_f1:.4f}")

    return mixture_vector / np.sum(mixture_vector), valid_loss, valid_accuracy, valid_f1
...
def main():
    ...
    rows = []
    for _ in range(50):
        mixture, valid_loss, valid_accuracy, valid_f1 = run_experiment(X_train, y_train, cluster_assignments_train, X_valid, y_valid, num_clusters)
        rows.append({
            "mixture": mixture,
    ...
    df.to_csv("/home/romainlhardy/code/hyperbolic-cancer/data/cellxgene/climb_mlp_results.csv", index=False)
